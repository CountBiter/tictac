...
public class PlayWindow extends JFrame implements ActionListener{
    ...
    PlayWindow(int CHOIX_LEVEL) {
        ...
        if (this.CHOIX_LEVEL == CHOIX_FRIEND)
            this.setTitle("Играть с другом");
        else if (this.CHOIX_LEVEL == CHOIX_EASY)
            this.setTitle("Играть с легким пк");
        else if (this.CHOIX_LEVEL == CHOIX_MEDIUM)
            this.setTitle("Играть со средним кп");
        else
            this.setTitle("Играть со сложным пкd");
        ...
        btnReset = new JButton("Перезапуск");
        ...
        btnBackToMain = new JButton("Вернуться в меню");
    ...
    private void printScore(int x, int o) {
        ...
        TableOfScore.setText("<html><table border='1'><tr><th colspan='2'>Счет :&nbsp;&nbsp;&nbsp;</th></tr>"
    ...
    private boolean getResult(boolean Player1Win) {
        if (((buttonsXO[0].getText().equals(buttonsXO[3].getText())) && (buttonsXO[0].getText().equals(buttonsXO[6].getText())) && (!buttonsXO[0].getText().equals("")))
            ...
            if (Player1Win) {
                xScore++;
                if (CHOIX_LEVEL == CHOIX_FRIEND)
                    JOptionPane.showMessageDialog(null, "<html>" + setColorOnly("Игрок 1 (X) ", "green") + setColorOnly("Выйграл !" , "green")  + "</html>");
                else
                    JOptionPane.showMessageDialog(null, setColor("Выйграли  ^_^ !" , "green"));
            } else {
                oScore++;
                if (CHOIX_LEVEL == CHOIX_FRIEND)
                    JOptionPane.showMessageDialog(null, "<html>" + setColorOnly("Игрок 2 (O) ", "blue") + setColorOnly("Выйграл !" , "green")  + "</html>");
                else
                    JOptionPane.showMessageDialog(null, "<html>" + setColorOnly("Проиграл : (", "red") + "<br>PC " + setColor("Выйграл !", "green") + "</html>");
            }
            printScore(xScore, oScore);
            clear();
            return false;
